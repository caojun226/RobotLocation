...
namespace RobotLocation.Service
{
    internal class ToneOp
    {
        ...
        private static void ProcessIOO()
        {
            ...
            {
                Thread.Sleep(1);
                if (RQO.TryDequeue(out RQnum)&& RQnum>6)
                {
             
                        for (int i = 3; i < 6; i++)
                        {
                        if (IOSignalFlags[i].TryGetValue(RQnum - 2, out int ResInt))
                        {
                            LogNet.Info($"相机{i}, 拍照次数{RQnum},取结果: {RQnum - 2}, 信号值: {ResInt} )");
                            ...
                            WENYU_EIO32P.Program.WY_WriteOutPutBit(1, i, 1);
                        }
                        else
                        {
                            LogNet.Warn($"相机{i}, 拍照次数{RQnum - 2}的信号值不存在");
                        }
                    }
                    }
                } 
        ...
        private static void ProcessIOI()
        {
            ...
            {
                Thread.Sleep(1);
                if (RQI.TryDequeue(out curOrder))
                {
                    for (int i = 0; i < 3; i++)
                    {
                        IOSignalFlags[i].TryGetValue(curOrder, out int ResInt);
        ...
        public static void DetermineRejection(Product p, int camera, int cardid, HTuple[] BatchResults)
        {
            try
            {
                ...
                if (p.Res == null)
                {
                    HOperatorSet.TupleGenConst(32, 0, out p1); // 如果结果为空，生成一个长度为32的全零HTuple
                ...
                if (p.Order >= 4)
                {
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        if (shouldReject)
                        {
                            Signal |= (1 << (i)); // 设置对应的二进制位,对应到刷新值
                            Signal = ~Signal; // 按位取反
                        }
                    }
                }
                // 存储信号值到IOSignalFlags
                IOSignalFlags[camera].AddOrUpdate(p.Order, Signal & 0xFF, (key, oldValue) => Signal & 0xFF);
                // 输出到日志
            ...
            if (RiseData != 0xffff)
            {
                ...
                if ((RiseData & 0x0002) == 0x0000) // 检测DI01上升沿
                {
                    if ((InPutData & 0x0002) == 0x0002)
                    {
                        ...
                        //Thread.Sleep(1);
                        //WENYU_EIO32P.Program.WY_SetLowOutPutData(1, 255);
                    }
        ...
        public static void Destroy()
        {
            try
            {
                ...
                foreach (var p in ToneProcesses)
                {
                    p.Destroy(); // 调用每个 ToneProcess 的销毁方法
                }
                ...
                {
                    SignalManagerThread.Interrupt(); // 中断线程
                    SignalManagerThread.Join(); // 等待线程结束
                }
